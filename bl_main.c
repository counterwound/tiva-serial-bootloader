...
ConfigureDevice(void)
{
    ...
    HWREG(SYSCTL_RCC) &= ~(SYSCTL_RCC_SYSDIV_M);
    Delay(524288);
    ...
    HWREG(GPIO_PORTB_BASE + GPIO_O_PCTL)  |= I2C_PINS_PCTL;
    ...
    HWREG(GPIO_PORTA_BASE + GPIO_O_PCTL)  |= SSI_PINS_PCTL;
    ...
    while(1)
    {
        ...
        switch(g_pui8DataBuffer[0])
        {
            ...
            case COMMAND_RUN:
            {
                ...
                //
#ifdef I2C_ENABLE_UPDATE
                HWREG(SYSCTL_RCGCI2C) &= ~SYSCTL_SRI2C_R0;
                HWREG(SYSCTL_SRI2C) = SYSCTL_SRI2C_R0;
                HWREG(SYSCTL_SRI2C) = 0;
#endif
#ifdef UART_ENABLE_UPDATE
                HWREG(SYSCTL_RCGCUART) &= ~SYSCTL_RCGCUART_R0;
                HWREG(SYSCTL_SRUART) = SYSCTL_SRUART_R0;
                HWREG(SYSCTL_SRUART) = 0;
#endif
#ifdef SSI_ENABLE_UPDATE
                HWREG(SYSCTL_RCGCSSI) &= ~SYSCTL_RCGCSSI_R0;
                HWREG(SYSCTL_SRSSI) = SYSCTL_RCGCSSI_R0;
